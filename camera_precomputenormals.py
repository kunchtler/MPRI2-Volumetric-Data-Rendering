...
def normal_to_isosurface(u):
    ...
    g_x = (interpolate_closest(ux_m1) - interpolate_closest(ux_p1)) / (ux_p1[0] - ux_m1[0])
    g_y = (interpolate_closest(uy_m1) - interpolate_closest(uy_p1)) / (uy_p1[1] - uy_m1[1])
    g_z = (interpolate_closest(uz_m1) - interpolate_closest(uz_p1)) / (uz_p1[2] - uz_m1[2])
...
precomputed_normals = np.zeros((data_resolution[0], data_resolution[1], data_resolution[2], 3))

def precompute_normals():
    #On précalcule les normales en chaque point de la grille, et on obteindra la normale d'un sample
    #En interpolant les normales précalculées.
    print('Precomputing normals.')
    for i in range(data_resolution[0]):
        for j in range(data_resolution[1]):
            for k in range(data_resolution[2]):
                precomputed_normals[i, j, k] = normal_to_isosurface(np.array([i, j, k]))
    print('Normals precomputed.')
...
def compute_color(dir_r, pt_d, pt_f):
    ...
    for i in range(n):
        ...
        sample_val = interpolate(sample_pos, data)
    ...
    for i in range(n):
        ...
        N = interpolate(sample_pos, precomputed_normals)
...
def main():
    ...
    #stats.print_stats()
    stats.dump_stats(filename='precomputed1.prof')
    show_image(image)
